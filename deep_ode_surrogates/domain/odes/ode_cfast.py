...
class CFASTODE(BaseODE):
    ...
    def _dynamics(self, x, t=None):
        ...
        dp = (
            (self.params.gamma - 1) / self.params.total_volume * (q_dot_u + q_dot_l)
        )  # pressure equation

        # Compute law of perfect gaz m_i = M_i * P * V_i / (R * T_i)
        M = 29.0  # Hypothesis : only air
        V_l = self.params.total_volume - x[3]
        m_l = M * x[0] * V_l / (self.params.R * x[2])
        m_u = M * x[0] * (x[3]) / (self.params.R * x[1])

        m_dot_l, m_dot_u = self.compute_source_terms_mass_flux()
        cst_u = self.params.cp * m_u
        dT_u = (1.0 / cst_u) * (
            q_dot_u - self.params.cp * m_dot_u * x[1] + x[3] * dp
        )  # upper temperature

        cst_l = self.params.cp * m_l

        dT_l = (1.0 / cst_l) * (
            q_dot_l - self.params.cp * m_dot_l * x[2] + V_l * dp
        )  # lower temperature
        dV_u = (1 / (x[0] * self.params.gamma)) * (
            (self.params.gamma - 1) * (q_dot_u) - x[3] * dp
        )  # upper volume
    ...
    def compute_source_terms_heat_flux(self, x):
        ...
        q_dot_l = (
            -self.params.cp
            * self.params.combustion_speed
            * self.params.combustion_heat
            * self.params.Q
            * x[2]
        )  # power
        q_dot_u = (
            self.params.combustion_speed * self.params.combustion_heat
            + self.params.cp * self.params.combustion_speed * self.params.outside_temperature
        )
        q_dot_u += (
            self.params.cp
            * self.params.combustion_speed
    ...
    def compute_source_terms_mass_flux(self):
        ...
        """
        m_dot_u = (
            self.params.combustion_speed * self.params.combustion_heat * self.params.Q
            + self.params.combustion_speed
        )
