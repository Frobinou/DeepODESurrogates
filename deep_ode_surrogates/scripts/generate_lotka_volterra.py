...
from deep_ode_surrogates.infrastructure.data.parquet.parquet_dataset_writer import (
...
fig = plot_trajectory(
    df["t"],
    y=df[["prey", "predator"]].values,
    state_names=["Prey", "Predator"],
    title="Lotka-Volterra Trajectory",
)
fig.show()

fig_phase = plot_phase_space(
    y=df[["prey", "predator"]].values,
    x_idx=0,
    y_idx=1,
    state_names=["Prey", "Predator"],
